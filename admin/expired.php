    ...
    <?php
        if(isset($_SESSION['login_user']))
        {
          ...
          if(isset($_POST['submit']))
          {
            ...
            mysqli_query($db, "INSERT INTO `fine` (`username`,`bookid`, `returned`, `day`, `fine`, `status`)VALUES('$_POST[username]', '$_POST[bookid]', '$x', '$day', '$fine', 'not paid') ;"); 
    ...
    $c=0;
      ...
      if(isset($_SESSION['login_user']))
      {
        ...
        $exp='EXPIRED';
          ...
          {
              $sql="SELECT student.username,roll,books.bookid,name,authors,edition,approve,issue,issue_book.return FROM student inner join issue_book ON student.username=issue_book.username inner join books ON issue_book.bookid=books.bookid WHERE issue_book.approve !='$exp'  ORDER BY `issue_book`.`return` DESC";
              $res=mysqli_query($db,$sql);
          }  
          else if(isset($_POST['submit3']))
          {
              $sql="SELECT student.username,roll,books.bookid,name,authors,edition,approve,issue,issue_book.return FROM student inner join issue_book ON student.username=issue_book.username inner join books ON issue_book.bookid=books.bookid WHERE issue_book.approve !='$ret' ORDER BY `issue_book`.`return` DESC";
              $res=mysqli_query($db,$sql);
          }
          else
          {
            $sql="SELECT student.username,roll,books.bookid,name,authors,edition,approve,issue,issue_book.return FROM student inner join issue_book ON student.username=issue_book.username inner join books ON issue_book.bookid=books.bookid WHERE issue_book.approve !='' and issue_book.approve !='Yes' ORDER BY `issue_book`.`return` DESC";
            $res=mysqli_query($db,$sql);
          }
